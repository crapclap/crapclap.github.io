import React from 'react'
import {
  Container,
  Typography
} from '@material-ui/core';

import "../../../common/common.css"
import { LyricStyles } from '../../_template/styles'

export const LoveMeDo = () => {

  const classes = LyricStyles();

  return (
    <div>
      <Container maxWidth="sm">
        <Typography variant="h6" align='left' display="block" className={classes.song_title}>
          Love me do
        </Typography>
        <Typography variant="body2" align='left' className={classes.lyric}>

          <div className={classes.lyric_block}>
            まだまだまだまだ足りない想い溢れて<br />
            それとは裏腹に進む時計の針と<br />
            にらめっこして勝てるはずもない<br />
            魔法でも使えりゃな ああ<br />
          </div>
          <div className={classes.lyric_block}>
            だらだらだらだら渋滞だ会話のペース<br />
            どっちが先に折れるかシーソーゲーム<br />
            好きだよなんて言えるはずもない<br />
            そんなハードデイズナイト<br />
            いつだってこうなんだ<br />
          </div>
          <div className={classes.lyric_block}>
            目を見て話せないのは<br />
            そこに恋が生まれた証だね<br />
          </div>
          <div className={classes.lyric_block}>
            ああ<br />
            どうしても君には言えないなこんなこと<br />
            言わなきゃなにも始まんないのは<br />
            わかってんだよ<br />
            ああ　愛してる<br />
            こんな5文字が言えない弱虫<br />
            故にラブミードゥー<br />
          </div>
          <div className={classes.lyric_block}>
            くるくるくるくるもうだめだ<br />
            君の目見れないな<br />
            僕の中に潜む恋の魔物騒いで<br />
            I wanna be your man<br />
            言えるはずもない<br />
            ラブラブラブ<br />
            抱きしめてキスしたいのに<br />
          </div>
          <div className={classes.lyric_block}>
            僕の全てを君に捧げてみたら<br />
            どうなるんだろうな<br />
            週に8日あれば君に好きと言えるのに<br />
            いつまでだって言えるはずもないのに<br />
            愛が金で買えたなら<br />
          </div>
          <div className={classes.lyric_block}>
            ああ<br />
            どうしても君には言えないなこんなこと<br />
            言わなきゃなにも始まんないのは<br />
            わかってんだよ<br />
            ああ　愛してる<br />
            こんな5文字が言えない弱虫<br />
            故にラブミードゥー<br />
          </div>
          <div className={classes.lyric_block}>
            ゆるい坂を下る<br />
            君の声がする方へ<br />
            嘘も幻も君なら抱きしめたい<br />
            いちご畑で永遠に<br />
          </div>
        </Typography>
      </Container>
    </div>
  );
};

export const TwilightRhapsody = () => {

  const classes = LyricStyles();

  return (
    <div>
      <Container maxWidth="sm">
        <Typography variant="h6" align='left' display="block" className={classes.song_title}>
          トワイライト・ラプソディー
        </Typography>
        <Typography variant="body2" align='left' className={classes.lyric}>

          <div className={classes.lyric_block}>
            暮れなずめば思い出してる<br />
            コーラの泡空に溶けていく<br />
            君がいた街<br />
          </div>
          <div className={classes.lyric_block}>
            なんの気もないくせに隣に<br />
            真昼の月みたいな笑顔<br />
            止まらぬドキドキに汗が滲む<br />
            君のワンピースは綺麗で<br />
            とても似つかわしくないとか<br />
            考えるだけ無駄なワインディングロード<br />
          </div>
          <div className={classes.lyric_block}>
            トワイライトが照らす街<br />
            どうでもいいことばかりに気が散る<br />
            恋に落ちる音が聞こえて<br />
            浮かんで消えそうになる夏のファンタジー<br />
            抱きしめて眠る夜<br />
          </div>
          <div className={classes.lyric_block}>
            トワイライトが照らす街<br />
            どうでもいいことばかりに気が散る<br />
            恋に落ちる音が聞こえて<br />
            浮かんで消えそうになる夏のファンタジー<br />
            抱きしめて眠る夜<br />
          </div>
          <div className={classes.lyric_block}>
            トワイライトが照らす街<br />
            どうでもいいことばかりに気が散る<br />
            恋に落ちる音が聞こえて<br />
            浮かんで消えそうになる夏のファンタジー<br />
            抱きしめて眠る夜<br />
          </div>
          <div className={classes.lyric_block}>
            何色にも染まらぬ影と<br />
            どこへも行かない空の色<br />
            複雑骨折の雲の行方<br />
          </div>
          <div className={classes.lyric_block}>
            ダウンタウンでキスをして<br />
            いつまでも変わらずに<br />
            あの日のままでなんて<br />
            恥ずかしくて言えやしないよな<br />
            遠ざかる でも消えない 夏のランデブー<br />
            抱きしめて眠る夜<br />
          </div>
          <div className={classes.lyric_block}>
            隣にいるとドキドキで<br />
            見たいのに見れないその瞳を僕のものに<br />
            夕暮れ時じゃ物足りない<br />
            神様お願いその瞳を僕のものに どうか<br />
          </div>
          <div className={classes.lyric_block}>
            トワイライトが照らす街<br />
            どうでもいいことばかりに気が散る<br />
            恋に落ちる5秒前の音<br />
            浮かんで消えそうになる夏のメロディー<br />
            抱きしめて眠る夜<br />
          </div>
          <div className={classes.lyric_block}>
            変わらずそこにいてね<br />
          </div>
        </Typography>
      </Container>
    </div>
  );
}

export const UnluckyStrike = () => {

  const classes = LyricStyles();

  return (
    <div>
      <Container maxWidth="sm">
        <Typography variant="h6" align='left' display="block" className={classes.song_title}>
          アンラッキーストライク
        </Typography>
        <Typography variant="body2" align='left' className={classes.lyric}>

          <div className={classes.lyric_block}>
          本日1本目はベランダで<br />
          起き抜けの身体を起こすためのもの<br />
          1日が始まる<br />
          </div>
          <div className={classes.lyric_block}>
          誰か想いながら吸う2本目はほろ苦く<br />
          やけに青い空は日曜<br />
          心雨模様<br />
          </div>
          <div className={classes.lyric_block}>
          ふいに火をつけた3本目<br />
          懐かしい街を歩く<br />
          あの日気をつけた坂歩けば 呼吸も速くなる<br />
          </div>
          <div className={classes.lyric_block}>
          誰か想って4本目<br />
          コンビニのスモーキングエリア<br />
          新しく出来たらしいな 研修中の店員さん<br />
          </div>
          <div className={classes.lyric_block}>
          変わってしまったな<br />
          気づいたらスーツなんか着て<br />
          思い出し笑いの１つもあればいいな<br />
          まだまだ歩けるかい<br />
          まだ火をつけたばかり<br />
          あと何本吸ったら君に会えるかな<br />
          </div>
          <div className={classes.lyric_block}>
          そりゃラークに過ごせればいいけど<br />
          クールな目してピースな言葉<br />
          最低最高な瞬間のハイライト<br />
          もう一度見れたりしないかな<br />
          </div>
          <div className={classes.lyric_block}>
          夜に照らされ5本目の煙よ月まで届け<br />
          もう優しくしなくていいよ<br />
          そのまま消えてよ<br />
          誰か想いながら吸い殻になり灰皿へGO<br />
          僕は生きている<br />
          たばこを吸っている<br />
          </div>
        </Typography>
      </Container>
    </div>
  );
}